...
void MassSpringSystem::updateTerrainPose(const double time) { 
    this->planeTerrain->updatePose(time);
    this->bowlTerrain->updatePose(time);
}

Eigen::Vector3f MassSpringSystem::getPlaneTerrainPose() { return this->planeTerrain->getPose(); }
Eigen::Vector3f MassSpringSystem::getBowlTerrainPose() { return this->bowlTerrain->getPose(); }
...
void MassSpringSystem::computeJellyForce(Jelly& jelly) {
    ...
    for (int i = 0; i < particleNum; i++) {
        ...
        float f = p.getForce().norm();
        if (f > 3000) {
            hasStrongCollision = true;
            return;
        }
    }
    hasStrongCollision = false;
